...
import { getCustomer, deleteCustomer } from '../../store/action/customerAction';
...
const Customer = () => {
  ...
  const { allCustomer, loading: allCustomerLoader } = useSelector((state) => {
          ...
          <div className="m-2 md:m-10 mt-24 p-2 md:p-10 bg-white rounded-3xl">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-3 gap-6">
              ...
              <div className="flex items-center justify-center py-2 sm:justify-end">
                <Button variant="contained" onClick={() => navigate('/customer/add-customer')}>+ Add New</Button>
              </div>
            </div>
            ...
            <Paper>
              <TableContainer sx={{ maxHeight: 500 }} component={Paper}>
                <Table sx={{ minWidth: 650 }} stickyHeader>
                  ...
                  <TableBody>
                    {allCustomer?.items?.map((row, index) => (
                      <TableRow
                        key={row._id}
                      ...
                      >
                        ...
                        <TableCell align="center">{row?.role == 'user' ?
                          <Chip label="user" style={{ backgroundColor: '#4BB543', color: '#ffffff' }} /> :
                          <Chip label="admin" style={{ backgroundColor: '#F44336', color: '#ffffff' }} />
                        }</TableCell>
                        <TableCell align="center">
                          <IconButton
                            onClick={() => navigate(`/customer/edit-customer/${row?._id}`)}
                          >
              ...
              <TablePagination
                ...
                component="div"
                count={parseInt(allCustomer?.count) || 0}
                rowsPerPage={rowsPerPage}
                page={!parseInt(allCustomer?.count) || parseInt(allCustomer?.count) <= 0 ? 0 : page}
